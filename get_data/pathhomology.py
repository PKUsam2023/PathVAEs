...
def input_cloudpoints(data_file, save_path, args):
    ...
    for alist in loadData[:1]:
        cloudpoints_name = alist[0]
        cloudpoints = alist[1]
        points_weight = alist[2]
        max_path = args.max_path
        result_path = save_path + cloudpoints_name
...
def input_digraph(data_file, save_path, args):
    loadData = np.load(data_file, allow_pickle=True)
    for alist in loadData:
        digraph_name = alist[0]
        cloudpoints = alist[1]
        edges = alist[2]
        all_edges = []
        result_path = save_path + digraph_name
        start_n = [i[0] for i in edges]
        end_n = [i[1] for i in edges]
        all_edges.append(start_n)
        all_edges.append(end_n)
        all_edges = np.array(all_edges).T - 1
        max_path = args.max_path
        if args.filtration_type == 'distance':
            PH = PathHomology()
...
def parse_args(args):
    ...
    parser.add_argument('--input_type', default='digraph', type=str, choices=['cloudpoints', 'digraph', 'No'])
    parser.add_argument('--input_data', default='./ML_HEA_100_simulation.npy', type=str,
    ...
    parser.add_argument('--filtration_type', default='distance', type=str, choices=['angle', 'distance'])
    parser.add_argument('--max_distance', default=7, type=float, help='if filtration_type is angle, it will be ignored')
    parser.add_argument('--save_name', default='./ML_HEA_100_simulation', type=str)
